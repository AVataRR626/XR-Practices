...
{
    public enum InputMode { Grip, Trigger, Joystic, ButtonPrimary, ButtonSecondary};
    ...
    private bool releaseFlag = false;
    ...
    public InputFeatureUsage<bool> UsageModeBool(InputMode im)
    {

        if (im == InputMode.ButtonPrimary)
            return CommonUsages.primaryButton;

        if (im == InputMode.ButtonSecondary)
    ...
    void Update()
    {
        ...
        if(inputMode == InputMode.ButtonPrimary || inputMode == InputMode.ButtonSecondary) 
            HandleButton();

        if(inputMode == InputMode.Grip || inputMode == InputMode.Trigger)
            HandleTriggerFloat();
    }
    ...
    public void HandleButton()
    {
        ...
        foreach (var device in validDevices)
        {
            ...
            tempState = device.TryGetFeatureValue(UsageModeBool(inputMode), out primaryButtonState) // did get a value
                        && primaryButtonState // the value we got
                        || tempState; // cumulative result from other controllers
        }

        if(lastButtonState != tempState)
        {
            if(tempState)
            {
                OnPress.Invoke();
        ...
        if (tempState)
        {
            pressValueRelay.value = 1;
            pressValue = 1;
        }
        else
        {
            pressValueRelay.value = 0;
            pressValue = 0;
        }
